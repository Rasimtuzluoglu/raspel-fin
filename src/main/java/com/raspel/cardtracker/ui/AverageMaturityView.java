...
public class AverageMaturityView extends VerticalLayout {
    ...
    private final Div uploadArea = new Div();
    ...
    public AverageMaturityView() {
        ...
        title.getStyle().set("margin", "0 0 4px 0").set("font-size", "22px").set("font-weight", "700");

        Span info = new Span("CSV yükleyin. Kolonlar: Bayi, Banka, Tutar, Taksit, Kart Açıklaması, Tarih, İşlem Sonucu");
        info.getStyle().set("color", "var(--lumo-secondary-text-color)").set("font-size", "13px").set("display", "block").set("margin-bottom", "16px");
        ...
        container.add(toolbar, info, upload, resultBox, grid);
        add(container);
    }

    private void configureGrid() {
        grid.setWidthFull();
        grid.getStyle().set("max-height", "350px").set("margin-top", "16px");
    ...
    private List<VadeRow> parseCSV(InputStream is) throws Exception {
        ...
        for (int i = 1; i < lines.length; i++) {
            ...
            try {
                ...
                sira++;
                rows.add(new VadeRow(sira, tarih, tutar, banka, kartAciklama, islemSonucu));
    ...
    private void showResult() {
        ...
        stats.add(buildStatCard("📋", "Toplam İşlem", basariliSayi + " adet"));
        stats.add(buildStatCard("💰", "Toplam Tutar", FormatUtils.formatNumber(toplamTutar) + " ₺"));
        stats.add(buildStatCard("📅", "Ortalama Vade", ortalamaVadeGun + " gün"));
    ...
    public record VadeRow(int sira, LocalDate tarih, BigDecimal tutar, String banka, String kartAciklama, String islemSonucu) {}
}
